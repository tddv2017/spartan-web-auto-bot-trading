...
import { Radar, List } from 'lucide-react';
import SignalFeed from '@/components/dashboard/SignalFeed'; // Đảm bảo Đại tá đã tạo file này từ trước
    ...
    <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <div className="xl:col-span-1 space-y-4">
            <div className="bg-green-900/10 border border-green-500/30 p-4 rounded-2xl">
                <h3 className="text-green-500 font-bold flex items-center gap-2 mb-2 text-sm uppercase tracking-wider"><Radar className="animate-spin-slow" size={18}/> LIVE SIGNALS</h3>
        ...
        <div className="xl:col-span-2">
            <div className="bg-slate-900/60 border border-slate-800 rounded-[2rem] p-6 h-full min-h-[500px] flex flex-col">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="font-bold text-slate-300 flex items-center gap-2 uppercase text-sm tracking-wider"><List size={16} className="text-blue-500"/> Lệnh đã khớp</h3>
                    <span className="text-[10px] text-slate-500 bg-black/30 px-2 py-1 rounded border border-slate-800">Page {currentPage}/{Math.ceil(trades.length / itemsPerPage) || 1}</span>
                </div>
                ...
                {trades.length > 0 ? (
                    <>
                                ...
                                <tbody className="divide-y divide-slate-800">
                                    {trades.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((trade, idx) => (
                                        <tr key={idx} className="hover:bg-slate-800/30 transition-colors">
                                            ...
                                            <td className="py-3"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${trade.type === 'BUY' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>{trade.type}</span></td>
                                            <td className="py-3 text-slate-500">{new Date(trade.time).toLocaleTimeString('vi-VN')}</td>
                                            <td className={`py-3 text-right pr-4 font-bold font-mono ${trade.profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>{trade.profit > 0 ? '+' : ''}{trade.profit.toFixed(2)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                        ...
                        {trades.length > itemsPerPage && (
                            <div className="flex justify-center items-center gap-2 mt-4 pt-4 border-t border-slate-800/50">
                                ...
                                {Array.from({ length: Math.ceil(trades.length / itemsPerPage) }, (_, i) => i + 1).map(pageNum => (<button key={pageNum} onClick={() => setCurrentPage(pageNum)} className={`w-6 h-6 rounded text-xs font-bold transition-all ${currentPage === pageNum ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}>{pageNum}</button>))}
                    ...
                    <div className="flex-grow flex items-center justify-center text-slate-500 italic">Chưa có dữ liệu giao dịch</div>
                )}
            </div>
        </div>
    </div>
