...
interface BotData {
  ...
  symbol: string;
  orders?: Order[]; // Danh sách lệnh của bot
}
...
export default function BattlefieldDashboard() {
  ...
  // ✅ 1. STATE QUẢN LÝ POP-UP CHI TIẾT
  ...
  return (
    <div className="min-h-screen bg-black text-green-500 font-mono p-4 md:p-6 relative overflow-hidden">
            ...
            <table className="w-full text-left text-xs">
                ...
                <tbody className="divide-y divide-green-900/30">
                    {displayBots.map(bot => (
                        <tr 
                          key={bot.id} 
                          onClick={() => setSelectedBot(bot)} // ✅ CLICK ĐỂ MỞ POP-UP
                        ...
                        >
                            ...
                            <td className="p-3 text-right font-mono">
                                ...
                                <div className={`text-[10px] ${bot.floatingProfit >= 0 ? 'text-green-600' : 'text-red-700'}`}>
                                    ({bot.floatingProfit.toFixed(2)} Float)
                                </div>
                            </td>
      ...
      {/* ✅ 2. MODAL HUD: CHI TIẾT LỆNH CHIẾN BINH */}
      {selectedBot && (
          ...
          <div className="w-full max-w-4xl bg-black border-2 border-green-500 shadow-[0_0_50px_rgba(34,197,94,0.2)] overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-4 border-b-2 border-green-500 bg-green-500/10 flex justify-between items-center">
              <div className="flex items-center gap-4">
                <div className="p-2 bg-green-500 text-black rounded-full animate-pulse">
                  <Crosshair size={24} />
                </div>
                <div>
              ...
              <button 
                ...
                className="p-2 hover:bg-red-500 hover:text-white transition-all text-green-500 border border-green-500"
              >
                <X size={20} />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div className="border border-green-900 p-3 bg-green-950/10">
                  ...
                  <p className="text-xl font-black text-white">${selectedBot.balance.toLocaleString()}</p>
                </div>
                <div className="border border-green-900 p-3 bg-green-950/10">
                  <p className="text-[10px] text-green-700 font-bold uppercase">Equity</p>
                  <p className="text-xl font-black text-white">${selectedBot.equity.toLocaleString()}</p>
                </div>
                <div className="border border-green-900 p-3 bg-green-950/10">
                  ...
                  <p className={`text-xl font-black ${selectedBot.floatingProfit >= 0 ? 'text-green-400' : 'text-red-500'}`}>
                    ${selectedBot.floatingProfit.toFixed(2)}
                  </p>
                </div>
                <div className="border border-green-900 p-3 bg-green-950/10">
                  <p className="text-[10px] text-green-700 font-bold uppercase">Symbol</p>
                  <p className="text-xl font-black text-blue-400">{selectedBot.symbol}</p>
                </div>
              </div>

              <h3 className="text-sm font-black text-green-500 mb-4 flex items-center gap-2">
                <Activity size={16} /> LIVE POSITIONS ({selectedBot.orders?.length || 0})
              </h3>
                ...
                <table className="w-full text-left text-[11px]">
                  ...
                  <tbody className="divide-y divide-green-900/30">
                    {(!selectedBot.orders || selectedBot.orders.length === 0) ? (
                      <tr>
                        <td colSpan={6} className="p-10 text-center text-green-800 italic uppercase font-bold">
                          No Active Orders Found in Signal Range.
                        </td>
                      </tr>
                    ) : (
                      selectedBot.orders.map((order) => (
                        <tr key={order.ticket} className="hover:bg-green-900/10 transition-colors">
                          ...
                          <td className="p-3 text-right font-bold text-white">{order.volume.toFixed(2)}</td>
                          <td className="p-3 text-right text-slate-400">{order.openPrice.toFixed(5)}</td>
                          <td className="p-3 text-right text-slate-400">{order.currentPrice.toFixed(5)}</td>
            ...
            <div className="p-4 border-t-2 border-green-500 bg-green-500/5 text-center">
                <p className="text-[9px] text-green-800 font-black tracking-[0.4em] uppercase">:: End of Tactical Data Transmission ::</p>
            </div>
          </div>
        </div>
      )}
