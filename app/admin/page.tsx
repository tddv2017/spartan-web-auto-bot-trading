...
import { ShieldAlert, RefreshCw, Infinity, Search, Wallet, CheckCircle, XCircle, UserPlus, LayoutDashboard, Users, Banknote, Activity, Server, Trash2, Zap, Crown, Play, Pause } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const updateUserSoldier = async (userId: string, currentExpiry: any, days: number, plan: string) => {
    ...
    try {
        if (!adminUser) return alert("❌ Lỗi: Chưa đăng nhập Admin!");
  ...
  const toggleUserStatus = async (user: any) => {
      ...
      const newMessage = isPaused ? "MARKET STABLE" : "⚠️ ADMIN PAUSED YOUR BOT";
      ...
      try {
          await updateDoc(doc(db, "users", user.id), {
              ...
              intelMessage: newMessage,
              lastAdminAction: new Date().toISOString()
          });
          setUsers(prevUsers => prevUsers.map(u => 
              u.id === user.id ? { ...u, remoteCommand: newCommand, intelMessage: newMessage } : u
          ));
          alert(`✅ Đã chuyển lệnh thành công: ${newCommand}`);
      } catch (e) {
  ...
  const handleApproveUser = async (user: any) => {
     ...
     try {
         ...
         fetchUsers(); alert("✅ Đã duyệt thành công!");
     } catch (e) { alert("Lỗi: " + e); }
  };

  const handleRejectUser = async (user: any) => {
     if(!confirm("TỪ CHỐI TÂN BINH NÀY?")) return;
     try { await updateDoc(doc(db, "users", user.id), { accountStatus: 'rejected', rejectedAt: new Date().toISOString() }); fetchUsers(); } catch (e) { alert("Lỗi: " + e); }
  };
                                            ...
                                            <div className="flex flex-col gap-1">
                                                <div className="flex items-center gap-2">
                                                    {u.remoteCommand === "PAUSE" ? (
                                                        <span className="text-red-500 font-bold text-[10px] uppercase border border-red-500/50 px-1.5 py-0.5 rounded bg-red-500/10 flex items-center gap-1"><Pause size={10}/> PAUSED</span>
                                                    ) : (
                                                        <span className="text-green-500 font-bold text-[10px] uppercase border border-green-500/50 px-1.5 py-0.5 rounded bg-green-500/10 flex items-center gap-1"><Play size={10}/> RUNNING</span>
                                                    )}
                                                </div>
                                                <div className="text-slate-400 text-xs">
                                                    {u.plan === 'LIFETIME' ? <Infinity size={14} className="text-purple-500"/> : u.expiryDate ? new Date(u.expiryDate.seconds * 1000).toLocaleDateString('vi-VN') : '---'}
                                                </div>
                                            </div>
                                        </td>
                                            ...
                                            <div className="flex justify-end gap-1">
                                                <button 
                                                    ...
                                                    className={`p-1.5 border rounded transition-all ${
                                                        ...
                                                        : "bg-orange-600/10 border-orange-600/30 text-orange-500 hover:bg-orange-600 hover:text-white"
                                                    }`} 
                                                    title={u.remoteCommand === "PAUSE" ? "Resume Bot" : "Pause Bot"}
                                                >
                                                    {u.remoteCommand === "PAUSE" ? <Play size={14}/> : <Pause size={14}/>}
                                                </button>
...
// // ==================================================================
// // ☢️ COMPONENT: BẢNG ĐIỀU KHIỂN KHẨN CẤP (BẢN NÂNG CẤP THÔNG MINH)
// // ==================================================================
// const EmergencyPanel = ({ onRefresh, adminUser }: { onRefresh: () => void, adminUser: any }) => {
//   const [loading, setLoading] = useState(false);

//   // 🔵 1. BACKUP DỮ LIỆU (Giữ nguyên logic cũ của Đại tá)
...
//   // 🔴 2. LỆNH DỪNG BẮN TOÀN DIỆN (CEASEFIRE)
//   // Thay vì cắt License, ta dùng remoteCommand để Bot MT5 vẫn quản lý lệnh cũ
//   const handleEmergencyStop = async () => {
//     if (!confirm("⚠️ KÍCH HOẠT TRẠNG THÁI NGỪNG BẮN TOÀN HỆ THỐNG?\n\nBot sẽ dừng vào lệnh mới nhưng vẫn giữ SL/TP cho khách.")) return;
//     setLoading(true);
//     try {
...
//             remoteCommand: "PAUSE",
//             intelMessage: "⚠️ HIGH IMPACT NEWS - PAUSED",
...
//       if (count > 0) { 
//         await batch.commit(); 
//         alert(`🚨 ĐÃ PHÁT LỆNH NGỪNG BẮN ĐẾN ${count} CHIẾN HẠM!`); 
//       } 
//       onRefresh(); 
//     } catch (e) { alert("❌ Lỗi: " + e); }
//     setLoading(false);
...
//   // 🟢 3. KHÔI PHỤC TẤN CÔNG (RESUME)
//   const handleRestore = async () => {
//     if (!confirm("✅ KHÔI PHỤC TRẠNG THÁI CHIẾN ĐẤU TOÀN HỆ THỐNG?")) return;
//     setLoading(true);
//     try {
//       const batch = writeBatch(db);
//       const querySnapshot = await getDocs(collection(db, "users"));
...
//       if (count > 0) { 
//         await batch.commit(); 
//         alert(`⚔️ TẤT CẢ ${count} BINH SĨ ĐÃ TIẾP TỤC QUÉT TÍN HIỆU!`); 
//       }
//       onRefresh();
//     } catch (e) { alert("❌ Lỗi!"); }
//     setLoading(false);
...
//           </div>
//           <div>
//             <h3 className="text-2xl font-black text-red-500 italic tracking-tighter uppercase">
//               Supreme Command Console
//             </h3>
//             <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.2em]">Điều khiển hỏa lực khẩn cấp cho toàn bộ 8 binh sĩ</p>
//           </div>
//         </div>
//         <div className="flex gap-4">
//           <button onClick={handleBackup} disabled={loading} className="px-8 py-4 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl shadow-xl flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50">
//             <span className="text-xl">💾</span> BACKUP
//           </button>
//           <button onClick={handleEmergencyStop} disabled={loading} className="px-8 py-4 bg-orange-600 hover:bg-orange-500 text-white font-black rounded-2xl shadow-xl flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50 border-b-4 border-orange-800">
//             <ZapOff size={20}/> CEASEFIRE (PAUSE ALL)
//           </button>
//           <button onClick={handleRestore} disabled={loading} className="px-8 py-4 bg-green-600 hover:bg-green-500 text-white font-black rounded-2xl shadow-xl flex items-center gap-2 transition-all active:scale-95 disabled:opacity-50 border-b-4 border-green-800">
//             <Zap size={20}/> ENGAGE (RUN ALL)
//           </button>
//         </div>
//       </div>
//     </div>
