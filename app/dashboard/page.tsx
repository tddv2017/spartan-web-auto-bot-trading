...
import { 
  ...
  Home, ChevronLeft, Terminal, PlayCircle // 👈 Thêm icon mới
} from 'lucide-react';
...
function DashboardContent() {
  ...
  // Logic mở modal thanh toán từ URL
  useEffect(() => {
  ...
  // Màn hình chờ khi chưa load xong profile
  if (!profile && user) {
    return (
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white font-sans selection:bg-green-500/30">
      ...
      <nav className="border-b border-slate-800 bg-slate-900/50 backdrop-blur-md px-6 py-4 flex justify-between items-center sticky top-0 z-50">
        ...
        >
          <LogOut size={16} /> <span className="hidden sm:inline">RÚT QUÂN</span>
        </button>
      </nav>
      ...
      <div className="max-w-6xl mx-auto p-4 md:p-8 space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
          ...
          <div className="flex flex-wrap gap-3">
            {profile?.plan && profile?.plan !== "free" && (
              ...
              >
                <CreditCard size={16} className="group-hover:rotate-12 transition-transform" /> GIA HẠN
              </button>
            )}
            ...
            >
              <Zap size={18} fill="currentColor" /> NÂNG CẤP
            </button>
          </div>
        </div>

        {/* 3. LICENSE CARD (Thẻ bài) */}
        <div className="bg-slate-900 border border-slate-800 p-6 md:p-8 rounded-[2rem] shadow-2xl relative overflow-hidden group hover:border-green-500/30 transition-colors">
          <div className="relative z-10">
            <h3 className="text-slate-500 font-bold uppercase text-[10px] mb-4 tracking-[0.2em] flex items-center gap-2">
              <Activity size={12} className="text-green-500"/> Kích hoạt License tại MT5
            </h3>
            <div className="flex flex-col md:flex-row gap-4 md:items-center">
              ...
              >
                ...
                {copied ? "ĐÃ SAO CHÉP" : "COPY KEY"}
              </button>
            </div>
            <p className="text-slate-500 text-xs italic mt-4">* Copy mã này dán vào mục Input của Bot Spartan trên MT5.</p>
          </div>
        ...
        {profile?.plan && profile?.plan !== "free" ? (
          <div className="bg-gradient-to-r from-green-900/20 to-slate-900 border border-green-500/30 p-8 rounded-[2rem] flex flex-col md:flex-row items-center justify-between gap-6 shadow-[0_0_30px_rgba(34,197,94,0.05)] relative overflow-hidden">
            ...
            <div className="flex items-center gap-5 relative z-10">
              ...
              <div>
                ...
                <p className="text-green-400 text-xs font-bold uppercase tracking-widest mt-1">Đã mở khóa tải xuống</p>
              </div>
            </div>
            <a 
          ...
          <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-[2rem] text-center">
            <p className="text-slate-500 text-sm">Vui lòng nâng cấp gói để tải Bot.</p>
          </div>
        )}
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-3xl p-6 md:p-8">
          ...
          <div className="space-y-6">
            <Step 
              num="01" 
              title="Cài đặt vào MT5" 
              desc="Copy file 'Spartan_Final.ex5' vào thư mục MQL5/Experts trên phần mềm MT5." 
            />
            <Step 
              num="02" 
              title="Cấu hình WebRequest" 
              desc="Vào Tools > Options > Expert Advisors. Tích chọn 'Allow WebRequest' và thêm URL:"
              code="https://spartan-web-auto-bot-trading.vercel.app/" 
            />
            <Step 
              num="03" 
              title="Nhập License Key" 
              desc="Kéo Bot vào biểu đồ XAUUSD (M15). Tại mục Input, dán License Key của bạn vào." 
            />
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-800 text-center">
            <button className="text-slate-400 hover:text-white flex items-center justify-center gap-2 mx-auto transition-colors text-sm hover:underline">
              <PlayCircle size={16} /> Xem Video hướng dẫn chi tiết
            </button>
          </div>
        </div>
